 ...
 */
public class CopyOfGameRankActivity extends Activity implements BaseActivity, OnClickListener {
			...
			if (msg.what == Const.MSG_GET_SUCCESS) { // 读取成功
				...
				try {
					...
					for (int i = 0; i < array.length(); i++) {
						...
						L.d("dd",
								"page ==" + page + "&&list.size=" + list.size() + "&&position="
				...
				ToastUtil.show(CopyOfGameRankActivity.this, R.string.egame_request_wrong);
				if (page == 1) {
					loading.showReload();
				}else{
					footer.showReload();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new TestGameListAdapter(list, CopyOfGameRankActivity.this, 2, downloadFromer, false);
	...
	public void initView() {
		loading = new Loading(CopyOfGameRankActivity.this);
	...
	public void initEvent() {
		...
		lvRank.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int arg1) {
				L.d("view", view.getLastVisiblePosition() + "|" + view.getCount()+"||footer size="+lvRank.getFooterViewsCount()+"||total="+totalRecord);
				if (isLastPage) {
					// 如果没有下一页了,就不再进行滚动加载
					L.d("view", "is last page");
				} else {
		...
		lvRank.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				Intent intent = new Intent(CopyOfGameRankActivity.this, GamesDetailActivity.class);
	...
	public void onClick(View v) {
		if (v == tvWeekRank && !tvWeekRank.isSelected()) {
			...
			UIUtils.buttonStateChange(mViewList, tvMonthRank, mOnPreDrawListener);
			cachelist.addAll(list);
			list.clear();
			adapter.notifyDataSetChanged();
			new ReleaseListIcon<GameListBean>(cachelist).release();
			...
			UIUtils.buttonStateChange(mViewList, tvTotalRank, mOnPreDrawListener);
			cachelist.addAll(list);
			list.clear();
			adapter.notifyDataSetChanged();
			new ReleaseListIcon<GameListBean>(cachelist).release();
	...
	void excute() {
		...
		runnable = new GetStringRunnable(handler, Urls.getGameRankUrl(CopyOfGameRankActivity.this, rankType, page,
				application.getUA()));
		// if (lvRank.getFooterViewsCount() == 0) {
		// lvRank.addFooter();
		// lvRank.onFootRefreshComplete(13);
